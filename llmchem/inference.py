...
def gen_text_stop_word(prompt, model, tokenizer,
    ...
    for i in range(max_tokens):
        ...
        generated_text = tokenizer.decode(
            input_ids[0], skip_special_tokens=True)[len(prompt):]
...
def ask_value(prompt, model, tokenizer, gpt_mode=False):
    if not gpt_mode:
        res = gen_text_stop_word(prompt, model, tokenizer)
    else:
        res = ask_gpt(prompt, model)
...
def ask_gpt(prompt, model="gpt-3.5-turbo-1106"):
    system_prompt = "You are a professional chemist, who only output ##Reason and ##Prediction. Complete the text according to the given examples. Never output anything else."
